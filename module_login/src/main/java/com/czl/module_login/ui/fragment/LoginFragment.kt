...
class LoginFragment : BaseFragment<LoginFragmentLoginBinding, LoginViewModel>() {
    ...
    override fun initData() {
        ...
        if (!TextUtils.isEmpty(SpHelper.decodeString(AppConstants.SpKey.APP_ACCOUNT))){
            viewModel.account.set(SpHelper.decodeString(AppConstants.SpKey.APP_ACCOUNT))
        }

        if (!TextUtils.isEmpty(SpHelper.decodeString(AppConstants.SpKey.APP_PASSWORD))){
            viewModel.pwd.set(SpHelper.decodeString(AppConstants.SpKey.APP_PASSWORD))
        }
//        initBtnState()
    }
