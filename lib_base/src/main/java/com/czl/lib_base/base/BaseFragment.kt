...
//        Messenger.getDefault().unregister(viewModel)
    ...
    open fun initStatusBar() {
        ImmersionBar.with(this)
            ...
            .statusBarColor(R.color.defaultTextColor).init()
    }

    open fun isThemeStatusBar(): Boolean {
        return false
    }
    ...
    open fun initFitThemeStatusBar() {
        if (!DayModeUtil.isNightMode(requireContext())) {
            ImmersionBar.with(this)
                ...
                .statusBarColor(R.color.base_color_1570ff)
                .fitsSystemWindows(true)
                .init()
        } else {
            ImmersionBar.with(this)
                .statusBarDarkFont(false, 0.2f)
                .statusBarColor(R.color.defaultTextColor)
                .fitsSystemWindows(true)
                .init()
        }
